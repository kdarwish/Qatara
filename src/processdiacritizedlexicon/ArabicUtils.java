...
public class ArabicUtils {
    ...
    public ArabicUtils()
    {
        String[] stop = {"و", "ما", "هي", "هو", "هم", "هما", "هن", "هذا", "هذه", "هذان", "هؤلاء", "هل", "في", "هنا", "هناك", "مع", "من", "علي", "كيف", "كان"};
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (s.substring(i, i + 1).matches("[" + ArabicUtils.ALLDelimiters + "]")) {
                ...
                if (i == 0) {
                    ...
                    if ((s.substring(i, i + 1).matches("[0-9]") && s.substring(i - 1, i).matches("[" + ArabicUtils.AllArabicLetters + ArabicUtils.buck2utf8("aiouNKF~") + "]"))
                            || (s.substring(i - 1, i).matches("[0-9]") && s.substring(i, i + 1).matches("[" + ArabicUtils.AllArabicLetters + ArabicUtils.buck2utf8("aiouNKF~") + "]"))) {
                        sFinal += " " + s.substring(i, i + 1);
                    } else {
                        sFinal += s.substring(i, i + 1);
                    }
